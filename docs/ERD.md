...
> Конверт события соответствует `EVENTS-001` (ecom-schema-registry).
...
```mermaid
erDiagram
  OUTBOX_EVENTS ||--o| ENVELOPE : "event_id = envelope.event_id"
  INBOX_EVENTS ||--o| ENVELOPE : "event_id = envelope.event_id"
...
entities:
  - name: OutboxEvent
    ...
    fields:
      ...
      - name: attempts
        ...
        default: 0
        check: "attempts >= 0"
        description: Число попыток публикации relay-ом
      - name: created_at
        type: timestamptz
        default: now()
        nullable: false
  ...
  - name: Envelope
    table: null
    description: Логическая модель конверта события (не таблица) — EVENTS-001, feasibility «предлагаемый envelope»
    fields:
      - name: event_id
        type: uuid
        nullable: false
...
relationships:
  - from: OutboxEvent
    ...
    cardinality: N:1
    label: serializes
    note: Строка outbox_events содержит один конверт в поле payload; event_id таблицы == event_id конверта
  - from: InboxEvent
    to: Envelope
    cardinality: N:1
    label: received_as
    note: event_id таблицы == Nats-Msg-Id доставленного сообщения == event_id конверта
```
...
- INV-5 (FR-005, BDD-033#S-9, S-11): dead-letter сохраняет исходный payload и метаданные (причина отказа, число попыток); петли «dead-letter → основной поток» нет.
...
| FR-005 | S-9, S-11 | dead-letter (payload + метаданные), отсутствие петли |
| FR-006 | S-10 | порядок commit → ack (ADR-001) |
| FR-007 | S-11 | inbox_events (INV-2) + redelivery после рестарта |

## 8. Открытые вопросы

- [ ] Q-1 (PRD-033 Q-1): формат `Nats-Msg-Id` — UUID `event_id` или хэш (aggregate_id + aggregate_version)? ERD зафиксировал `event_id` (uuid) как канонический ключ; влияет на FR-002, до старта unit-тестов.
- [ ] Q-2 (BDD-033 Q-1): использовать ли native JetStream dedup (Nats-Msg-Id) в дополнение к inbox-таблице? На уникальный constraint inbox не влияет.
- [ ] Q-3 (BDD-033 Q-2): формат метаданных dead-letter — envelope-поля или отдельные headers? Влияет на INV-5.
- [ ] Q-4 (PRD-033 Q-2): fault injection — в production-код relay или test-only билд? Влияет на тесты FR-007.
